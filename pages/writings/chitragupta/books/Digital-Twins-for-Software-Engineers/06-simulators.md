...
- what the Functional Mock-up Interface (FMI) actually is (Chapter 5,
  Sec. 5.7.1);
...
about soil.** Figure 6.1 is why.

```
   two timescales in one system

   tube filling    |||||||||||||||||||||||||||||||||||||||
   (seconds)       every one of these needs a step

   soil drying     |         |         |         |
   (days)          this is the behaviour you care about

   |<------------------- 7 days --------------------->|

   an explicit stepping rule must satisfy the FASTEST:

        h  <=  about 1 s      ->    604,800 steps
        h  =   15 min         ->        672 steps
                                   ~900x the work, same answer
                                   about the soil

   The fix is not a faster machine. It is one of:
     - an implicit solver (pays per step, needs far fewer)
     - do not model the tube at all (Sec. 6.9.2's choice)

  Figure 6.1  Stiffness. The step size is hostage to the fastest
              behaviour in the model, even when the question is
              about the slowest.
```


That is stiffness, and it is why the word is a budget warning. Note the
second escape route in the figure: the cheapest fix for a stiff system is
often to stop modelling the fast part, which is a modelling decision rather
than a solver one.
...
configuration, or another criterion -- with the system's mode and both its
discrete and its continuous variables all liable to change at each
...
compliant answers -- reported as the commonest route to a hybrid
physics-and-machine-learning model [14].
...
Figure 6.2 traces one turn of that cycle, and the thing to watch is where
the estimate lands: never on the model's prediction, never on the reading,
...
```
   moisture
      ...
      +----------------------------------------> t
        ...
        x  predict:  advance the model from the last *
        o  correct:  pull toward the new reading

   Trust the sensor more  ->  * sits nearer o
   Trust the model more   ->  * sits nearer x

  Figure 6.2  One predict-correct cycle. The estimator's output is
              a third quantity, and believing either input on its
              own is the mistake Sec. 6.6.1 opens with.
```


That third quantity is the answer to "which do you believe?", and it is why
Chapter 3 drew the state estimator as its own component rather than folding
it into the store: it produces something neither the connector nor the model
has.
...
[17], and it is not confined to teaching examples. One
reported offshore deployment builds its twin on physical models, checks the
result against measurements from a real turbine at full size, and uses a
Kalman filter over a linearised model to recover structural quantities that
are not directly instrumented [18].
...
Mock-up Interface (FMI)** is the widely used standard for that. Models are
packaged as **Functional Mock-up Unit (FMU)** components, which can then be
handed between tools and stepped alongside one another
[24]. The standard's contribution is a fixed set of
calls: an FMU exposes them, and whatever environment is orchestrating the
run drives the FMU through them -- usually with several other models being
driven the same way at the same time [25]. Its practical value is breadth: a broad span of
modelling and simulation tools support it and can export models as
...
Figure 6.3 draws the same split as a question about where the solver box
sits, which is the form you can check against a supplier's deliverable.

```
   FMI for CO-SIMULATION          FMI for MODEL EXCHANGE
   ...
   |        FMU          |        +---------------------+
   |  +---------------+  |        |        FMU          |
   |  |    SOLVER     |  |        |   equations only    |
   |  +---------------+  |        |   "here is dW/dt"   |
   |  equations          |        +---------------------+
   +---------------------+

   solver travels WITH the model   solver stays with the TOOL

   you get: a black box that       you get: equations you can
   steps itself; vendor keeps      step your way; you own the
   its numerical choices           numerical choices, and the
                                   blame

  Figure 6.3  The one distinction in FMI. Ask a supplier which
              variant they ship and you have learned who owns the
              solver -- and therefore who owns the step size.
```


That is the split, stated in the standard's own terms: an FMU either
carries its own solver or leaves the numerical integration to the
simulation environment [25]. The practical consequence is
Chapter 5's: whoever owns the solver owns the step size, and the step size
is what Sec. 5.4.4 showed can turn a wrong answer into a meaningless one.
...
[12] Z. Ali, R. Biglari, J. Denil, J. Mertens, M. Poursoltan, and M. K. Traoré, "From modeling and simulation to Digital Twin: evolution or revolution?," *SIMULATION*, vol. 100, no. 7, pp. 751–769, 2024.
