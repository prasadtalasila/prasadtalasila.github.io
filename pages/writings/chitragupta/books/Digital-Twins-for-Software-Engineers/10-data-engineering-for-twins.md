...
data spaces -- Chapter 15. The Findable, Accessible, Interoperable and
Reusable (FAIR) principles, and data standards -- Chapter 13. Feature
...
```
  ...
  Figure 10.2  Why bounded nearest is the default. The unbounded
              join always returns something, and what it returns
              across a gap is fiction with a measured value's
              type.
```
...
Figure 10.1 draws the four on two axes, which is the picture that makes
"what did we believe on 14 August?" a different question from "what was
true on 14 August?".

```
   decision_time / ingest_time
   (when WE knew)
        ^
        |
   Aug  |                    o  belief formed 20 Aug
   20   |                   /   about 14 Aug
        |                  /    (a correction)
        |                 /
   Aug  |          o     /      belief formed 14 Aug
   14   |         /     /       about 14 Aug
        |        /     /
        |       /     /
        +------+-----+---------------> as_of / event_time
              Aug   Aug              (when the WORLD was)
              14    14

   the diagonal  = beliefs formed about the present
   above it      = beliefs formed about the past
                   (corrections, backfill, retroactive
                    invalidation -- Sec. 10.6.4)
   below it      = a forecast

   "what did we believe on 14 Aug?"  -> read a HORIZONTAL line
   "what is true of 14 Aug now?"     -> read a VERTICAL line

  Figure 10.1  Two times, two questions. One timestamp collapses
              the plane to the diagonal and makes the horizontal
              question unanswerable.
```


The two reading directions in that figure are the whole of Sec. 10.5.3's
worked alert, and they are why an audit ("why did you water then?") and a
diagnosis ("what actually happened?") need different queries against the
same table.
...
alert
 ...
 -> the decision_time and inputs_watermark it ran under
  ...
  Figure 10.3  The provenance chain for one alert, followed until
               it reaches something physical. Every arrow is a
               foreign key or a validity-interval lookup that
               Sec. 10.3 and Sec. 10.4 already built.
```

Read Figure 10.3 once for its depth rather than its detail. The alert is
four levels from a sensor's calibration record, and each level is a place
where the chain can be broken by a single unrecorded write.
...
from the other direction: data whose origin and method of computation stay
fully traceable is what lets the whole of it be linked together
...
- *Context as a first-class, changing thing:* [4]
  ...
  built around recording how the physical system is structured *and how
  that structure changes* -- with plants that move, which is Sec. 10.4's problem in its
  purest form. [13] takes the same line further
  into semantic technology than this chapter goes.
- *Provenance:* [6] on keeping origin and method of
  computation traceable throughout, and [7] on
...
[10] J. Robles, C. Martín, and M. Díaz, "OpenTwins: An open-source framework for the development of next-gen compositional digital twins," *Computers in Industry*, vol. 152, p. 104007, 2023.
